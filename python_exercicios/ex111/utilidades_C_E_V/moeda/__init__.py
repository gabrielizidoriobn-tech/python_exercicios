...
def resumo(p = 0, a = 0, d = 0):
    '''função resumo: engloba todas as funções ateriores em uma so
parametro p: valor numerico a ser calculado
parametro a: porcentagem a ser aumentada
parametro d: porcentagem a ser diminuida'''
    print('-'*30)
    print(f'{'RESUMO DO VALOR':^30}')
    print('-'*30)
    print(f'analisando o valor:\t{formato(p)}')
    print(f'''dobro do preço: \t{dobro(p, True)}
metade do preço:\t{metade(p,True)}
{a}% de aumento: \t{aumentar(p, a, True)}
{d}% de redução: \t{diminuir(p, d, True)}''')
